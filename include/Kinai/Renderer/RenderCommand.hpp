...
public:
	...
	static void	DrawIndexed(const Ref<VertexArray>& vertex_array, PrimitiveType mode, IndexType type, uint32_t index_count = 0)
	{
		_renderer_api->DrawIndexed(vertex_array, mode, type, index_count);
	}

	static void	Draw(const Ref<VertexArray>& vertex_array, PrimitiveType mode, uint32_t vertex_count)
	{
		_renderer_api->Draw(vertex_array, mode, vertex_count);
	}
